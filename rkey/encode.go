...
import (
	...
	"hg.sr.ht/~dchapes/ripple/encoding/base58"
)

var rippleEncoding = base58.Ripple
...
func encode(k Key) ([]byte, error) {
	...
	return rippleEncoding.AppendEncode(nil, tmp), nil
}
...
func decode(k Key, text []byte) (Key, error) {
	tmp, err := rippleEncoding.AppendDecode(nil, text)
	if err != nil {
		return nil, err
	}
	v := version(tmp[0])
